...
const pressReleases = [
  { date: "February 27, 2026", title: "Hanzo Launches One API for Every Model — 100+ Models, Zero Markup", description: "The Hanzo AI API provides access to over 100 models from every major provider through a single API key with zero markup on third-party pricing.", link: "/blog" },
  { date: "February 27, 2026", title: "Hanzo Cloud Launches with 10-54% Lower Pricing Than Competitors", description: "Hanzo Cloud offers developer VMs starting at $5/mo with 2x the RAM of DigitalOcean at the same price, zero egress fees, and global multi-cloud routing.", link: "/blog" },
  { date: "February 27, 2026", title: "Hanzo Bot: Full-Stack AI Agent Hosting from $5/mo", description: "Deploy AI agents with built-in access to 100+ models, 260+ MCP tools, GPU compute, and multimodal capabilities — audio transcription 85% cheaper than OpenAI.", link: "/blog" },
  { date: "February 27, 2026", title: "Zen4 Coder 480B: The Largest Open-Source Code Model", description: "Zen4 Coder 480B MoE delivers frontier-class code generation with only 22B active parameters, available through the Hanzo AI API.", link: "/blog" },
  { date: "February 9, 2026", title: "Zen4 Model Family Launch — 14 Models from 600M to 480B Parameters", description: "Zoo Labs Foundation releases the Zen4 model family built on Mixture of Diverse Experts architecture, spanning chat, code, vision, and audio — served on the Hanzo AI API.", link: "/blog" },
  { date: "February 27, 2026", title: "Complete AI Pricing Breakdown: Models, Cloud, GPU, and Audio", description: "A comprehensive reference for all Hanzo AI pricing across the gateway, cloud infrastructure, GPU compute, and multimodal services.", link: "/blog" },
]
          ...
          <Box className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-secondary/50 border border-border rounded-xl p-8">
              <Box className="flex items-center gap-3 mb-6">
                <Box className="w-10 h-10 rounded-lg flex items-center justify-center bg-primary/10"><Building2 className="w-5 h-5 text-foreground" /></Box>
              ...
              <Box className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {companyFacts.map((fact) => (
                  <Box key={fact.label} className="text-center bg-neutral-800/50 rounded-lg p-4">
                    <Box className="text-2xl font-bold text-foreground mb-1">{fact.value}</Box>
                    <Box className="text-sm text-muted-foreground">{fact.label}</Box>
                  </Box>
                ))}
              </Box>
            </motion.div>
            ...
            <Box className="grid md:grid-cols-2 gap-6">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-secondary/50 border border-border rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  <Box className="flex items-center gap-3 text-muted-foreground"><Phone className="w-5 h-5 text-foreground" />+1 (424) 335-0550</Box>
                </div>
              </motion.div>
            ...
            <Box className="grid md:grid-cols-3 gap-6 mb-8">
              {logoAssets.map((asset, index) => (
                <motion.div key={asset.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="rounded-xl border border-border overflow-hidden">
                  ...
                  <Box className="p-4 bg-secondary/50">
            ...
            <Box className="mt-10">
              ...
              <Box className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {partnerAssets.map((partner) => (
                  <Box key={partner.name} className="border border-border rounded-xl p-4 bg-secondary/40">
                    <Box className="text-sm uppercase tracking-widest text-muted-foreground mb-2">{partner.name}</Box>
                ...
                <motion.div key={release.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                  <Link href={release.link || "#"} className="block p-6 rounded-xl bg-secondary/50 border border-border hover:border-border transition-all group">
                    <Box className="flex items-start justify-between gap-4">
                      <div>
                        <Box className="text-sm text-muted-foreground mb-2">{release.date}</Box>
            ...
            <Box className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {socialLinks.map((social, index) => (
                <motion.a key={social.name} href={social.href || "#"} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="p-4 rounded-xl bg-secondary/50 border border-border text-center hover:border-border transition-all">
                  <Box className="font-semibold text-foreground mb-1">{social.name}</Box>
                  <Box className="text-sm text-muted-foreground">{social.handle}</Box>
                </motion.a>
              ))}
            </Box>
          </Box>
