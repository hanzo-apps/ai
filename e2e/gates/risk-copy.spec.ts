...
test('the lookback answers one party, and is credited with nothing else', () => {
  // `POST /v1/aml/relationships/search` resolves the name to a pseudonym and calls
  // retention.Ledger.Lookback. That reads the records indexed under ONE party,
  // keeps the ones whose Class is a relationship, and answers AMLR Art. 78:
  // Maintained, Current, the Natures found, the record ids, the window, and
  // Examined — which the ledger carries precisely to show the answer came from
  // the party index rather than a scan.
  //
  // It never reads a transaction: the loop skips every class but the one. And a
  // relationship record names the four pseudonyms of a SINGLE person — subject,
  // name, account, wallet — so there is no second party in it to walk to. So
  // "the relationships around a subject … which is where structuring across
  // several accounts becomes visible" credited this route with two things it
  // does not do: a traversal (there is no counterparty graph in the engine), and
  // a detection. Structuring across a customer's own accounts IS caught, by the
  // `structuring-accounts` rule at ingest — a different plane — and attributing
  // it here is the kind of claim that reaches support as "your product said it
  // would show me this". The hero and the prose already called this a lookback;
  // the card was the last place still telling the old story.
  const text = words()
  ...
  expect(text, 'an index lookup, which is what "fully and speedily" needs').toMatch(/does not grow with the ledger/i)
  ...
  // than to a character distance — a bound of "within N characters" is satisfied
  // by rewording, which is not the same as being true. A detection or a traversal
  // may not be named in the same breath as a relationship, because the lookback
  // does neither. Written this way the old sentence cannot come back in any
  // paraphrase, which a `not.toContain` of its text could not promise.
  const credited = text
    .split(/(?<=[.!?])\s+/)
