...
export function OSSRevenueBanner({ upstreamName, compact = false }: OSSRevenueBannerProps) {
  if (compact) {
    return (
      ...
      >
        <Box className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-secondary/30 px-4 py-3">
  ...
  return (
    ...
    >
      <Box className="max-w-5xl mx-auto">
        <Box className="border border-border rounded-xl bg-gradient-to-r from-secondary/30 to-secondary/10 p-6 md:p-8">
          <Box className="flex flex-col md:flex-row md:items-center gap-6">
            {/* Left content */}
            <Box className="flex-1">
              <Box className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-foreground/5 border border-border mb-4">
            ...
            {/* Right buttons */}
            <Box className="flex flex-col sm:flex-row md:flex-col gap-3 flex-shrink-0">
              <Link
                href="/open-source/dividends"
